  ...
  const fetchWards = useCallback(async () => {
        try {
                ...
                if (res.ok) {
                          ...
                          if (ward42) {
                                      setSelectedWardId(ward42.id);
                          }
                          setApiError(null);
                } else {
                          console.error("Failed to fetch wards: status", res.status);
                          setApiError("서버에 연결할 수 없습니다. 잠시 후 다시 시도해주세요.");
                }
        } catch (error) {
                console.error("Failed to fetch wards:", error);
                setApiError("서버에 연결할 수 없습니다. 네트워크 상태를 확인해주세요.");
        }
  }, []);
        ...
        try {
          ...
          const res = await fetch(`/api/nurses?${params.toString()}`);
                if (res.ok) {
                          ...
                          console.error("Failed to fetch nurses: status", res.status);
                }
        } catch (error) {
