...
export default function NurseFormModal({
  ...
  const validate = (): boolean => {
        ...
        if (!formData.wardId) {
                newErrors.wardId = "병동 정보를 불러올 수 없습니다. 페이지를 새로고침 해주세요.";
        }
  ...
  const isEdit = !!nurse;
    const wardLoadFailed = wards.length === 0;
                      ...
                      <div>
                                ...
                                <div
                                              className={`w-full rounded-lg border px-3 py-2 text-base ${
                                                              errors.wardId
                                                                ? "border-red-300 bg-red-50 text-red-700 dark:bg-red-900/30 dark:border-red-700 dark:text-red-300"
                                                                : "border-slate-200 bg-slate-50 text-slate-700 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100"
                                              }`}
                                            >
                                            42병동
                                </div>
